					...
					<div class="col-sm-9">
						...
						<div class="panel-body">
							...
							<table id="demo-table" class="table-striped" data-toolbar="#demo-custom-toolbar" data-sort-name="id" data-sort-order="desc" data-pagination="true" data-url="data/data-table.json" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1">
                                ...
                                <tbody>
                                    <?php
					$sql="SELECT * FROM `message` ORDER BY `created_on` DESC";
					$query= $handler->query($sql);
					$count = 1;
					while ($ray = $query->fetch()) {
						// echo "<tr><td>".$count."</td><td>".$ray['name']."</td>";
						echo "<tr><td>".$count."</td>";
						echo "<td <a href='msgs.php?msg_id={$ray['msg_id']}' class='' data-toggle='modal' data-target='#largeModal' style='cursor:pointer;'>".$ray['sender']."</a></td>";
						echo "<td>".$ray['created_on']."</td><td>";

						if (!$ray['is_delete']) {
							echo "<a href='?sp_id={$ray['msg_id']}&&soft_d={$ray['is_delete']}' class='alert bg-success col-xs-3 info' role='alert'style='cursor:pointer;'>A</a>";
						}else{
							echo "<a href='?sp_id={$ray['msg_id']}&&soft_d={$ray['is_delete']}' class='alert bg-danger col-xs-3 info' role='alert' style='cursor:pointer;'>D</a>";
						}
						// echo "</td><td>";
						echo "<a href='msgs.php?msg_id={$ray['msg_id']}' class='alert bg-warning col-xs-6 info' data-toggle='modal' data-target='#largeModal' role='alert' style='cursor:pointer;'>View</a>"; 
						echo "</td></tr>";
