<?php 
	...
	$all_spotlight =spotlight_images($handler,"SELECT * FROM `spotlight` ORDER BY `created_on` ASC LIMIT ".$page.",6");

	$rows_wanted = 6;
	$total_rows = $handler->query("SELECT * FROM `spotlight`");
	...
	while ($all = $all_spotlight->fetch()) {
		echo "<div class='col-xs-4'><div class='row'><img class='img img-responsive img-thumbnail gall_img' src='../spotlight/".$all['image']."' height='140px'></div><div class='row'>";
		if ($all['is_active']) {
			echo "<a href='?sp_id={$all['spot_id']}&&active={$all['is_active']}&&soft_d=5' class='alert bg-success col-xs-4 info' role='alert' style='cursor:pointer;'>Active</a>";
		}else{
			echo "<a href='?sp_id={$all['spot_id']}&&active={$all['is_active']}&&soft_d=5' class='alert bg-danger col-xs-4 info' role='alert' style='cursor:pointer;'>Not Active</a>";
		}
		if (!$all['is_delete']) {
			echo "<a href='?sp_id={$all['spot_id']}&&soft_d={$all['is_delete']}&&active=5' class='alert bg-success col-xs-6 info' role='alert' style='cursor:pointer;'>Available</a>";
		}else{
			echo "<a href='?sp_id={$all['spot_id']}&&soft_d={$all['is_delete']}&&active=5' class='alert bg-danger col-xs-6 info' role='alert' style='cursor:pointer;'>Soft Deleted</a>";
		}
		echo "</div><div class='row'><h4>".$all['title']."</h4></div></div>";
		$count ++;
		if ($count == 3) {
