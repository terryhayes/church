			...
			if($event_check){
                ...
                <div class='alert alert-success' role='alert'>
                    ...
                    <div class='media-body'>
                        ...
                        <p class='alert-message'>Event Created</p>
                    </div>
                </div>
            </div></div></div>";
			}
            ...
            $message = "<div id='page-alert'><div class='alert-wrap in'>
                <div class='alert alert-warning' role='alert'>
                    ...
                    <div class='media-body'>
                        ...
                        <p class='alert-message'>Programme With Such Name Already Exist</p>
                    </div>
                </div>
            </div></div></div>";
		}
					...
					<div class="col-sm-9">
							<div class="panel">
								<div class="panel-heading">
									<h3 class="panel-title">All Programme </h3>
								</div>
                                <?php echo $message;?>
                                <!--Data Table-->
						<!--===================================================-->
							...
							<table id="demo-table" class="table-striped" data-toolbar="#demo-custom-toolbar" data-sort-name="id" data-sort-order="desc" data-pagination="true" data-url="data/data-table.json" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1">
								<thead>
									<tr>
										...
										<th data-field="location" data-sortable="true" data-formatter="">Location</th>
										<th data-field="other" data-align="center" data-sortable="true" data-formatter="">Status</th>
                                        <th data-field="date" data-align="center" data-sortable="true" data-formatter="">Date</th>
									</tr>
								</thead>
                                <tbody>
                                    <?php
					$sql="SELECT * FROM `program` WHERE `prog_id`";
					$query= $handler->query($sql);
					$count = 1;
					while ($ray = $query->fetch()) {
						// echo "<tr><td>".$count."</td><td>".$ray['name']."</td>";
						echo "<tr><td>".$count."</td>";
						echo "<td <a href='events.php?prof_id={$ray['prog_id']}' class='' data-toggle='modal' data-target='#largeModal' style='cursor:pointer;'>".$ray['name']."</a></td>";
						echo "<td>".$ray['location']."</td><td>";

						if (!$ray['is_delete']) {
							echo "<a href='?sp_id={$ray['prog_id']}&&soft_d={$ray['is_delete']}' class='alert bg-success col-xs-3 info' role='alert'style='cursor:pointer;'>A</a>";
						}else{
							echo "<a href='?sp_id={$ray['prog_id']}&&soft_d={$ray['is_delete']}' class='alert bg-danger col-xs-3 info' role='alert' style='cursor:pointer;'>D</a>";
						}
						// echo "</td><td>";
						echo "<a href='events.php?prof_id={$ray['prog_id']}' class='alert bg-warning col-xs-6 info' data-toggle='modal' data-target='#largeModal' role='alert' style='cursor:pointer;'>View</a>"; 
						echo "</td><td>".$ray['created_on']."</td></tr>";
