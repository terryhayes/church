<?php
	...
	if (isset($_GET['sp_id'])) {
		...
		if(update_alb($handler,$sql_1,$status,$id)){
	...
	<div id="container" class="effect mainnav-lg">
		...
		<div class="boxed">
			...
			<div id="content-container">
				...
				<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
				<div id="page-title">
					<h1 class="page-header text-overflow">Albums</h1>
				</div>
				<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
				<!--End page title-->
				...
				<div id="page-content">
					...
					<div class="col-sm-9">
							<div class="panel">
								<div class="panel-heading">
									<h3 class="panel-title">All Albums</h3>
								</div>
                                <?php echo $message;?>
                                <!--Data Table-->
						<!--===================================================-->
							...
							<table id="demo-table" class="table-striped" data-toolbar="#demo-custom-toolbar" data-sort-name="id" data-sort-order="desc" data-pagination="true" data-url="data/data-table.json" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1">
								<thead>
									<tr>
										...
										<th data-field="location" data-sortable="true" data-formatter="">Description</th>
					...
					while ($ray = $query->fetch()) {
						echo "<tr><td>".$count."</td><td>".$ray['title']."</td>";
						echo "<td>".$ray['description']."</td><td><div>";

						if (!$ray['is_delete']) {
							echo "<a href='?sp_id={$ray['album_id']}&&soft_d={$ray['is_delete']}' class='alert bg-success col-xs-6  info' role='alert'style='cursor:pointer;'>A</a>";
						}else{
							echo "<a href='?sp_id={$ray['album_id']}&&soft_d={$ray['is_delete']}' class='alert bg-danger col-xs-6  info' role='alert' style='cursor:pointer;'>D</a>";
						}
						echo "</div>";
						// echo "<div><p class='row'>".$ray['created_on']."</p></div>";
						echo "</td></tr>";
